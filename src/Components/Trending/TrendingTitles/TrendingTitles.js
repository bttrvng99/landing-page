...
export default function TrendingTitles({
  ...
  return (
    <div className="flex flex-col gap-y-2">
      <TrendingThumbnail duration={data?.runtime} rating={rating} imgUrl={imgUrl} id={data?.id}/>
      <div className="flex flex-row items-center justify-between">
        <a href={'/movie/'+data?.id} className="text-ellipsis overflow-hidden whitespace-nowrap font-semibold text-2xl hover:text-red-600 transition-colors">
          {title}
        </a>
